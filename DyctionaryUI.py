...
class Dy_UserInterface(Dy_UserInterfaceInit):
    ...
    def Dy_ListenFunction(self, Dy_Listen):
        if self.Dy_ListenStart:
            ...
            self.Dy_ListenButtonI.set_from_file(self.Dy_IconDir + "Listen_Icon_Red_35.png")
            self.Dy_Spinner.start()
            self.Dy_SpinnerSpeak.stop()
            self.Dy_StatusBottomOne.set_label(self.Dy_StatusBottomOneText[1])

            self.Dy_SpeechRegThread = threading.Thread(target=self.Dy_SpeechReg)
    ...
    def Dy_SpeechReg(self, ):
        ...
        time.sleep(2.5)
        subprocess.Popen(['espeak', '-s', '120', '-v', 'f5', 'Are your ready?'], shell=False)

        while True:
            self.Dy_SpeechRegFile = open(self.Dy_SpeechRegPath + "Dy_SpeechReg.output", 'r')
            ...
            if self.Dy_SpeechRegInput == "READY LETS START " and self.Dy_ListenReady == False:
                self.Dy_StatusBottomTwo.set_label(self.Dy_StatusBottomTwoText[1])
                self.Dy_StatusBottomThree.set_label(self.Dy_StatusBottomThreeText[1])
                self.Dy_UserQueryText = "Cool.. what do you thinking?"
                subprocess.Popen(['espeak', '-s', '140', '-v', 'f3', 'Speak something'], shell=False)
                self.Dy_UserQuery.set_label(self.Dy_UserQueryText)
                time.sleep(5)
            ...
            if self.Dy_ListenReady:
                if self.Dy_SpeechRegInput == 'THIS IS COMMAND STOP LISTENING ':
                    ...
                    self.Dy_ListenButtonI.set_from_file(self.Dy_IconDir + "Listen_Icon_35.png")
                    self.Dy_Spinner.stop()
                    self.Dy_SpinnerSpeak.start()
                    self.Dy_ListenReady = False
                    self.Dy_ListenStart = True
                    self.Dy_StatusBottomOne.set_label(self.Dy_StatusBottomOneText[2])
                    ...
                    subprocess.Popen(['espeak', 'Listening have stopped'], shell=False)
                    break
                    ...
                    try:
                        ...
                        if self.Dy_SpeechRegInputPre != self.Dy_SpeechRegInput:
                            self.fixResult, check = self.Dy_UserEntryAnalyzerObject.Dy_Evaluation(self.Dy_SpeechRegInput.lower())
    ...
    def Dy_SpeakLoud(self, Dy_SpeakLoud, Dy_SpeakType):
        try:
            if Dy_SpeakType == 'S':
                subprocess.Popen(['espeak', '-s', '160', '-v', 'f5', self.Dy_ResponseSmallSpeech])
    
            if Dy_SpeakType == 'M':
                subprocess.Popen(['espeak', '-s', '145', '-v', 'f5', self.Dy_ResponseLargeSpeech])
        except:
            subprocess.Popen(['espeak', '-v', 'f5', '-s', '145', 'Unable to Read..'])
    ...
    def Dy_TextEntry(self, Dy_UserEntry):
        ...
        Dy_FinalLongTextResponse = self.Dy_UserEntryAnalyzerObject.Dy_ResponseInstant(Dy_TextEntryData.strip())
    ...
    def Dy_TextEntryFinal(self, Dy_UserEntry):
        ...
        self.Dy_UserEntryAnalyzerObject = Dy_UserEntryAnalyzer.Dy_UserEntryAnalyzer()
        Dy_TextEntryData = self.Dy_UserEntry.get_text()
        self.fixResult, checkExist = self.Dy_UserEntryAnalyzerObject.Dy_Evaluation(Dy_TextEntryData)
    ...
    def Dy_fixEverThing(self, fixList, checkExist):
        ...
        if len(fixList) == 1:
            ...
            subprocess.Popen(['espeak', '-v', 'f5', self.Dy_ResponseSmallSpeech])
            self.set_focus(self.Dy_UserEntry)
    ...
    def Dy_callMoreWindow(self, moreWindow):
        ...
        try:
            self.Dy_additionalWindow = subprocess.Popen(['python3',
            ...
            subprocess.Popen(['espeak', '-v', 'f5', 'your last search not supported'], shell=False)
    ...
    def Dy_CloseAll(self, Dy_SwitchOff):
        ...
        self.Dy_SpinnerSpeak.stop()
        self.Dy_ListenReady = True
        self.Dy_SpeechRegStop = open(self.Dy_SpeechRegPath + "Dy_SpeechReg.output", 'w')
